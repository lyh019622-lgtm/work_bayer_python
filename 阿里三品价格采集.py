...
def step1_data_cleaning(input_file):
    ...
    if missing_columns:
        ...
        df['到手_优惠价_清洗后'] = df['价格'].apply(clean_price)
...
def filter_ka_stores(df_sorted_all, input_file, sheet_name, candidate_stores):
    ...
    required_columns = ['商品详情链接', '店铺名称', '商品名称', '到手_优惠价_清洗后']
    ...
    output_file_path = os.path.join(output_dir, f"阿里{drug_name}KA{date_str}{ext}")

    try:
        # 输出KA文件时只保留指定的四列，并在商品名称列后面添加来源
        output_columns = ['商品名称', '商品详情链接', '店铺名称', '到手_优惠价_清洗后']
        ...
        # 在商品名称列后面添加来源列，值为阿里（第二列）
        output_df.insert(1, '来源', '阿里')
...
class KAStoreFilter:
    ...
    def filter_by_keywords(self, df, keyword1, keyword2):
        """
        根据商品名称中的关键词筛选数据
        ...
        筛选后的数据
        """
        def contains_both_keywords(text):
            """检查文本是否同时包含两个关键词"""
            if pd.isna(text):
                return False
            ...
            text_str = str(text)
            # 检查是否包含第一个关键词
            has_keyword1 = keyword1 in text_str

            # 检查是否包含第二个关键词
            has_keyword2 = keyword2 in text_str

            # 同时包含两者
            return has_keyword1 and has_keyword2

        condition = df['商品名称'].apply(contains_both_keywords)
        return df[condition]
    ...
    def find_lowest_price_per_store(self, df):
        ...
        if self.price_column not in df.columns:
            # 尝试寻找其他可能的价格列
            price_columns = ['价格', '单价', '售价', '成交价', '优惠价', '到手价', '到手_优惠价_清洗后']
    ...
    def process_file(self, input_file_path, sheet_name):
        ...
        if missing_columns:
            return False, f"缺少列: {', '.join(missing_columns)}", None, None
        ...
        filtered_yousiming_df = self.filter_by_keywords(store_filtered_df, '优思明', '21片')
        filtered_yousiyue_df = self.filter_by_keywords(store_filtered_df, '优思悦', '28片')
        lowest_price_yousiming_df = self.find_lowest_price_per_store(filtered_yousiming_df)
        lowest_price_yousiyue_df = self.find_lowest_price_per_store(filtered_yousiyue_df)

        filtered_dfs = {
            '优思明筛选结果': filtered_yousiming_df,
            '优思悦筛选结果': filtered_yousiyue_df,
            '优思明最低价格': lowest_price_yousiming_df,
            '优思悦最低价格': lowest_price_yousiyue_df
        }

        return True, "处理完成", None, filtered_dfs

    def save_results(self, input_file_path, filtered_dfs):
        """保存结果到Excel文件"""
        if not filtered_dfs:
            return None
        ...
        output_file_path = os.path.join(output_dir, f"阿里{drug_name}8KA{date_str}{ext}")
                ...
                if len(filtered_dfs['优思明筛选结果']) > 0:
                    ...
                    if '日期' in df.columns:
                        try:
                            ...
                            df = df.sort_values(['日期', '店铺名称'])
                        except:
                            pass
                    # 只保留指定列，并在商品名称列后面添加来源
...
def main():
    ...
    filter_ka_stores(df_sorted_all, input_file, sheet_name, candidate_stores)
    ...
    with pd.ExcelFile(temp_file) as xl:
        ...
        success, message, _, filtered_dfs = filter_tool.process_file(temp_file, sheet_name)

        if success and filtered_dfs:
            final_output_file = filter_tool.save_results(input_file, filtered_dfs)
